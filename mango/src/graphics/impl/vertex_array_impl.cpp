...
void vertex_array_impl::bind_vertex_buffer(int32 index, buffer_ptr buffer, int64 offset, int64 stride)
{
    MANGO_ASSERT(is_created(), "Vertex array not created!");
    MANGO_ASSERT(buffer->is_created(), "Buffer not created!");
    MANGO_ASSERT(index >= 0, "Cannot bind vertex buffer with negative index!");
    ...
    if (!potential_buffer.buf || potential_buffer.buf->get_name() != name)
    {
        ...
        glVertexArrayVertexBuffer(m_name, static_cast<g_uint>(index), name, static_cast<g_intptr>(offset), static_cast<g_sizei>(stride));
    }
}
...
void vertex_array_impl::set_vertex_attribute(int32 index, int32 buffer_index, format attribute_format, int64 relative_offset)
{
    MANGO_ASSERT(is_created(), "Vertex array not created!");
    MANGO_ASSERT(index >= 0, "Cannot set vertex attribute with negative index!");
    MANGO_ASSERT(buffer_index >= 0, "Cannot set vertex attribute for buffer with negative index!");
    ...
    if (it == hopefully_bound_buffer.enabled_attributes.end())
    {
        ...
        glEnableVertexArrayAttrib(m_name, static_cast<g_uint>(index));
        glVertexArrayAttribFormat(m_name, static_cast<g_uint>(index), size, type, normalized, static_cast<g_uint>(relative_offset));
        glVertexArrayAttribBinding(m_name, static_cast<g_uint>(index), static_cast<g_uint>(buffer_index));
    }
