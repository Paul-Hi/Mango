...
linux_input_system::linux_input_system(const shared_ptr<context_impl>& context)
{
...
bool linux_input_system::create()
{
    return true;
}

void linux_input_system::set_platform_data(const shared_ptr<platform_data>& data)
{
    PROFILE_ZONE;
...
void linux_input_system::update(float dt)
{
    MANGO_UNUSED(dt);
}

void linux_input_system::destroy() {}

input_action linux_input_system::get_key(key_code key)
{
...
input_action linux_input_system::get_mouse_button(mouse_button button)
{
    MANGO_ASSERT(m_platform_data->native_window_handle, "Window Handle is not valid!");
...
modifier linux_input_system::get_modifiers()
{
    MANGO_ASSERT(m_platform_data->native_window_handle, "Window Handle is not valid!");
    return m_last_mods;
}

glm::vec2 linux_input_system::get_mouse_position()
{
    MANGO_ASSERT(m_platform_data->native_window_handle, "Window Handle is not valid!");
    double x, y;
...
glm::vec2 linux_input_system::get_mouse_scroll()
{
    MANGO_ASSERT(m_platform_data->native_window_handle, "Window Handle is not valid!");
    return m_last_scroll_offset;
}

void linux_input_system::set_key_callback(key_callback callback)
{
    MANGO_ASSERT(m_platform_data->native_window_handle, "Window Handle is not valid!");
    m_input_user_data.key_change.connect(callback);
}

void linux_input_system::set_mouse_button_callback(mouse_button_callback callback)
{
    MANGO_ASSERT(m_platform_data->native_window_handle, "Window Handle is not valid!");
    m_input_user_data.mouse_button_change.connect(callback);
}

void linux_input_system::set_mouse_position_callback(mouse_position_callback callback)
{
    MANGO_ASSERT(m_platform_data->native_window_handle, "Window Handle is not valid!");
    m_input_user_data.mouse_position_change.connect(callback);
}

void linux_input_system::set_mouse_scroll_callback(mouse_scroll_callback callback)
{
    MANGO_ASSERT(m_platform_data->native_window_handle, "Window Handle is not valid!");
    m_input_user_data.mouse_scroll_change.connect(callback);
}

void linux_input_system::set_drag_and_drop_callback(drag_n_drop_callback callback)
{
    MANGO_ASSERT(m_platform_data->native_window_handle, "Window Handle is not valid!");
    m_input_user_data.drag_n_drop_change.connect(callback);
}

void linux_input_system::hide_cursor(bool hide)
{
    MANGO_ASSERT(m_platform_data->native_window_handle, "Window Handle is not valid!");
