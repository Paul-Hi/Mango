...
void glfw_display::change_size(int32 width, int32 height)
{
    ...
    m_glfw_display_data.info.m_width  = width;
    m_glfw_display_data.info.m_height = height;
    glfwSetWindowSize(m_glfw_display_data.native_handle, m_glfw_display_data.info.m_width, m_glfw_display_data.info.m_height);
}
...
{
    return m_glfw_display_data.info.m_x;
}

int32 glfw_display::get_y_position() const
{
    return m_glfw_display_data.info.m_y;
}

int32 glfw_display::get_width() const
{
    return m_glfw_display_data.info.m_width;
}

int32 glfw_display::get_height() const
{
    return m_glfw_display_data.info.m_height;
}

const char* glfw_display::get_title() const
{
    return m_glfw_display_data.info.m_title;
}

bool glfw_display::is_decorated() const
{
    return m_glfw_display_data.info.m_decorated;
}

display_configuration::native_renderer_type glfw_display::get_native_renderer_type() const
{
    return m_glfw_display_data.info.m_native_renderer;
}
...
bool glfw_display::initialize()
{
    ...
    switch (m_glfw_display_data.info.m_native_renderer)
    {
...
bool glfw_display::create_glfw_opengl()
{
    ...
    if (m_glfw_display_data.info.m_pixel_format == display_info::pixel_format::rgba8888)
        glfwWindowHint(GLFW_ALPHA_BITS, 8);

    switch (m_glfw_display_data.info.m_depth_stencil_format)
    {
    ...
    glfwWindowHint(GLFW_DECORATED, m_glfw_display_data.info.m_decorated ? GLFW_TRUE : GLFW_FALSE);

    GLFWmonitor* monitor = glfwGetPrimaryMonitor();

    m_glfw_display_data.native_handle = glfwCreateWindow(m_glfw_display_data.info.m_width, m_glfw_display_data.info.m_height, m_glfw_display_data.info.m_title, monitor, NULL);
    if (!m_glfw_display_data.native_handle)
    {
        const char* description;
        glfwGetError(&description);
        MANGO_LOG_ERROR("glfwCreateWindow failed! No dieplay is created! Error: {0}", description);
        return false;
    }

    glfwSetWindowUserPointer(m_glfw_display_data.native_handle, &m_glfw_display_data);

    glfwSetWindowPos(m_glfw_display_data.native_handle, m_glfw_display_data.info.m_x, m_glfw_display_data.info.m_y);

    // TODO Paul: Fancy icon please?
    // glfwSetWindowIcon(window, 2, images);

    // Event callback setup
    if (m_glfw_display_data.info.m_display_event_handler)
    {
        // Error callback
        ...
        glfwSetWindowPosCallback(m_glfw_display_data.native_handle, [](GLFWwindow* window, int x_pos, int y_pos) {
            ...
            data->info.m_x          = x_pos;
            data->info.m_y          = y_pos;
            data->info.m_display_event_handler->on_window_position(x_pos, x_pos);
        });
        ...
        glfwSetFramebufferSizeCallback(m_glfw_display_data.native_handle, [](GLFWwindow* window, int w, int h) {
            glfw_display_data* data = static_cast<glfw_display_data*>(glfwGetWindowUserPointer(window));
            data->info.m_display_event_handler->on_window_framebuffer_resize(w, h);
        });
        ...
        glfwSetWindowContentScaleCallback(m_glfw_display_data.native_handle, [](GLFWwindow* window, float x_scale, float y_scale) {
            glfw_display_data* data = static_cast<glfw_display_data*>(glfwGetWindowUserPointer(window));
            data->info.m_display_event_handler->on_window_content_scale(x_scale, y_scale);
        });
        ...
        glfwSetMouseButtonCallback(m_glfw_display_data.native_handle, [](GLFWwindow* window, int button, int action, int mods) {
            glfw_display_data* data = static_cast<glfw_display_data*>(glfwGetWindowUserPointer(window));
            data->info.m_display_event_handler->on_input_mouse_button(static_cast<mouse_button>(button), static_cast<input_action>(action), static_cast<modifier>(mods));
        });

        // Cursor position callback
        glfwSetCursorPosCallback(m_glfw_display_data.native_handle, [](GLFWwindow* window, double x_pos, double y_pos) {
            glfw_display_data* data = static_cast<glfw_display_data*>(glfwGetWindowUserPointer(window));
            data->info.m_display_event_handler->on_input_cursor_position(x_pos, y_pos);
        });

        // Cursor enter callback
        glfwSetCursorEnterCallback(m_glfw_display_data.native_handle, [](GLFWwindow* window, int entered) {
            glfw_display_data* data = static_cast<glfw_display_data*>(glfwGetWindowUserPointer(window));
            data->info.m_display_event_handler->on_input_cursor_enter(entered);
        });

        // Scroll callback
        glfwSetScrollCallback(m_glfw_display_data.native_handle, [](GLFWwindow* window, double x_off, double y_off) {
            glfw_display_data* data = static_cast<glfw_display_data*>(glfwGetWindowUserPointer(window));
            data->info.m_display_event_handler->on_input_scroll(x_off, y_off);
        });

        // Key callback
        glfwSetKeyCallback(m_glfw_display_data.native_handle, [](GLFWwindow* window, int key, int, int action, int mods) {
            glfw_display_data* data = static_cast<glfw_display_data*>(glfwGetWindowUserPointer(window));
            data->info.m_display_event_handler->on_input_key(static_cast<key_code>(key), static_cast<input_action>(action), static_cast<modifier>(mods));
        });

        // Drop callback
        glfwSetDropCallback(m_glfw_display_data.native_handle, [](GLFWwindow* window, int path_count, const char** paths) {
            glfw_display_data* data = static_cast<glfw_display_data*>(glfwGetWindowUserPointer(window));
            data->info.m_display_event_handler->on_input_drop(path_count, paths);
        });
    }

    MANGO_LOG_DEBUG("Created new opengl display!");
    MANGO_LOG_DEBUG("Display Position is ({0}, {1})", m_glfw_display_data.info.m_x, m_glfw_display_data.info.m_y);
    MANGO_LOG_DEBUG("Display Size is {0} x {1}", m_glfw_display_data.info.m_width, m_glfw_display_data.info.m_height);
