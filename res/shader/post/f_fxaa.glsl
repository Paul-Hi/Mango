...
void main()
{
    ...
    vec4 rgba_center  = textureLod(sampler_input, texcoord, 0.0);

    float luma_center = luma(rgba_center);
    float luma_north  = luma(textureLodOffset(sampler_input, texcoord, 0.0, ivec2( 0, -1)));
    float luma_west   = luma(textureLodOffset(sampler_input, texcoord, 0.0, ivec2(-1,  0)));
    float luma_east   = luma(textureLodOffset(sampler_input, texcoord, 0.0, ivec2( 1,  0)));
    float luma_south  = luma(textureLodOffset(sampler_input, texcoord, 0.0, ivec2( 0,  1)));
    ...
    float luma_north_west = luma(textureLodOffset(sampler_input, texcoord, 0.0, ivec2(-1, -1)));
    float luma_north_east = luma(textureLodOffset(sampler_input, texcoord, 0.0, ivec2( 1, -1)));
    float luma_south_west = luma(textureLodOffset(sampler_input, texcoord, 0.0, ivec2(-1,  1)));
    float luma_south_east = luma(textureLodOffset(sampler_input, texcoord, 0.0, ivec2( 1,  1)));
    ...
    float luma_end0 = luma(textureLod(sampler_input, sample_dir0, 0.0)) - half_luma0c;
    float luma_end1 = luma(textureLod(sampler_input, sample_dir1, 0.0)) - half_luma0c;
    ...
    {
        if(!done0) luma_end0 = luma(textureLod(sampler_input, sample_dir0, 0.0)) - half_luma0c;
        if(!done1) luma_end1 = luma(textureLod(sampler_input, sample_dir1, 0.0)) - half_luma0c;
    ...
    frag_color = vec4(textureLod(sampler_input, final_uv, 0.0).rgb, 1.0);
}
